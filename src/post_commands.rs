...
use crate::config::{Config, PostCommand, TemplateContext, ReplaceConfig};
...
impl<'a> PostCommandExecutor<'a> {
    ...
    async fn execute_post_command(&self, post_command: &PostCommand, index: usize) -> Result<()> {
        match post_command {
            PostCommand::Simple(command_str) => {
                let substituted_command = self.config.substitute_template_variables(command_str, &self.context);
                self.execute_command_string(&substituted_command, None, None, index).await
            },
            PostCommand::Complex(config) => {
                let substituted_command = self.config.substitute_template_variables(&config.command, &self.context);
                
                // Check condition if specified
                if let Some(ref condition) = config.condition {
                    if !self.evaluate_condition(condition)? {
                        log::debug!("Skipping command {} due to condition: {}", index + 1, condition);
            ...
            PostCommand::Replace(config) => {
                ...
                if let Some(ref condition) = config.condition {
                    if !self.evaluate_condition(condition)? {
                        log::debug!("Skipping replace {} due to condition: {}", index + 1, condition);
                        if let Some(ref name) = config.name {
                            println!("⏭️  Skipped: {}", name);
                        } else {
    ...
    async fn execute_command_string(
        ...
        if let Some(env_vars) = environment {
            for (key, value) in env_vars {
                let substituted_value = self.config.substitute_template_variables(value, &self.context);
    ...
    fn evaluate_condition(&self, condition: &str) -> Result<bool> {
        if let Some(file_path) = condition.strip_prefix("file_exists:") {
            let substituted_path = self.config.substitute_template_variables(file_path, &self.context);
            let full_path = self.working_dir.join(substituted_path);
            Ok(full_path.exists())
        } else if let Some(dir_path) = condition.strip_prefix("dir_exists:") {
            let substituted_path = self.config.substitute_template_variables(dir_path, &self.context);
            let full_path = self.working_dir.join(substituted_path);
            Ok(full_path.is_dir())
    ...
    async fn execute_replace_action(&self, config: &ReplaceConfig, _index: usize) -> Result<()> {
        let file_path = self.config.substitute_template_variables(&config.file, &self.context);
        let pattern = self.config.substitute_template_variables(&config.pattern, &self.context);
        let replacement = self.config.substitute_template_variables(&config.replacement, &self.context);
    ...
    pub fn print_template_variables(&self) {
        println!("📋 Available template variables:");
        println!("  {{branch_name}} = {}", self.context.branch_name);
        println!("  {{db_name}} = {}", self.context.db_name);
        println!("  {{db_host}} = {}", self.context.db_host);
        println!("  {{db_port}} = {}", self.context.db_port);
        println!("  {{db_user}} = {}", self.context.db_user);
        if self.context.db_password.is_some() {
            println!("  {{db_password}} = [configured]");
        }
        println!("  {{template_db}} = {}", self.context.template_db);
        println!("  {{prefix}} = {}", self.context.prefix);
    }
}
