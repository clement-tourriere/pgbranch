...
impl LocalStateManager {
    ...
    pub fn set_current_branch(&mut self, project_path: &Path, branch: Option<String>) -> Result<()> {
        let project_key = self.get_project_key(project_path)
            .ok_or_else(|| anyhow::anyhow!("Failed to get project key for path: {}", project_path.display()))?;
    ...
    pub fn cleanup_old_projects(&mut self, max_age_days: u32) -> Result<()> {
        ...
        let old_projects: Vec<String> = self.state.projects
            .iter()
    ...
    fn load_state(state_file_path: &Path) -> Result<LocalState> {
        ...
        let content = fs::read_to_string(state_file_path)
            .with_context(|| format!("Failed to read local state file: {}", state_file_path.display()))?;

        let state: LocalState = serde_yaml::from_str(&content)
            .with_context(|| format!("Failed to parse local state file: {}", state_file_path.display()))?;
    ...
    fn save_state(&self) -> Result<()> {
        ...
        fs::write(&self.state_file_path, content)
            .with_context(|| format!("Failed to write local state file: {}", self.state_file_path.display()))?;
...
mod tests {
    ...
    fn test_project_key_generation() {
        ...
        assert!(project_key.unwrap().contains(temp_dir.path().to_str().unwrap()));
    }
    ...
    fn test_current_branch_operations() {
        ...
        // Set a branch
        manager.set_current_branch(&config_path, Some("feature_test".to_string())).unwrap();
        assert_eq!(manager.get_current_branch(&config_path), Some("feature_test".to_string()));
        
        // Update branch
        manager.set_current_branch(&config_path, Some("main".to_string())).unwrap();
        assert_eq!(manager.get_current_branch(&config_path), Some("main".to_string()));
