...
impl DBLabBackend {
    ...
    async fn make_request<T: for<'de> Deserialize<'de>>(&self, method: reqwest::Method, path: &str, body: Option<&impl Serialize>) -> Result<T> {
        ...
        if !response.status().is_success() {
            ...
            anyhow::bail!("DBLab API request failed with status {}: {}", status, error_text);
        }
    ...
    async fn get_latest_snapshot(&self) -> Result<String> {
        let response: ListSnapshotsResponse = self.make_request(reqwest::Method::GET, "/api/snapshots", None::<&()>).await?;
    ...
    fn normalize_clone_name(branch_name: &str) -> String {
        // DBLab has specific naming requirements
        branch_name
            .to_lowercase()
            .chars()
...
impl DatabaseBranchingBackend for DBLabBackend {
    async fn create_branch(&self, branch_name: &str, from_branch: Option<&str>) -> Result<BranchInfo> {
        ...
        // If from_branch is specified, try to find its snapshot
        let snapshot_id = if let Some(from) = from_branch {
            // Find the clone to get its snapshot
            let clones = self.list_branches().await?;
            clones.into_iter()
                .find(|c| c.name == from)
                .map(|c| c.database_name) // Using database_name to store snapshot_id
        } else {
            None
        };

        // If no snapshot specified, use the latest one
        ...
        let response: CreateCloneResponse = self.make_request(reqwest::Method::POST, "/api/clones", Some(&request)).await?;
    ...
    async fn delete_branch(&self, branch_name: &str) -> Result<()> {
        ...
        // First find the clone by name to get its ID
        let clones: ListClonesResponse = self.make_request(reqwest::Method::GET, "/api/clones", None::<&()>).await?;
        let clone = clones.clones.into_iter()
    ...
    async fn list_branches(&self) -> Result<Vec<BranchInfo>> {
        let response: ListClonesResponse = self.make_request(reqwest::Method::GET, "/api/clones", None::<&()>).await?;

        let branches = response.clones.into_iter()
            .map(|clone| BranchInfo {
                name: clone.name.clone(),
                created_at: Some(clone.created_at),
                parent_branch: None, // DBLab doesn't track parent relationships
                database_name: clone.snapshot_id, // Store snapshot_id for reference
            })
            .collect();
    ...
    async fn switch_to_branch(&self, branch_name: &str) -> Result<BranchInfo> {
        ...
        // For DBLab, switching is handled by post_commands via connection string
    ...
    async fn test_connection(&self) -> Result<()> {
        // Test by listing clones
        let _: ListClonesResponse = self.make_request(reqwest::Method::GET, "/api/clones", None::<&()>).await?;
        Ok(())
    }
    ...
    fn supports_template_from_time(&self) -> bool {
        false // DBLab uses snapshots, not point-in-time
    }
